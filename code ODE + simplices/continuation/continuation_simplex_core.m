...
while i < n_iter
    ...
    if any(mod(i:i+length(index_new_simplices),250)==0)
        stored_index = store_center(save_file, save_file_nodes, list_of_frontal_nodes);
        
        for delete_i = 1:length(stored_index)
            index = stored_index(delete_i);
            list_of_nodes{index}=[];
        end
    end
...
if all(gap_angle_vec) == Inf
    fprintf('Impossible to continue growth, since all border nodes reached the cutoff')
    priority_node = [];
    return
end
