...
if isempty(norm_weight)
    norm_weight = ones(size_scalar+size_vector,1);
elseif length(norm_weight) ~= size_scalar+size_vector
    if length(norm_weight) == 1
        norm_weight = norm_weight * ones(size_scalar+size_vector,1);
    else
...
vector_weights = norm_weight(1+size_scalar:end);
if ~use_intlab
...
for j=1:size_vector
    for i=1:size_scalar
        ...
        norm_vec(j)=norm_vec(j)+NuNorm(Zji,nu)*norm_weight(j)/norm_weight(i);
    end
    %disp(Z)
    for i=1:size_vector
        Zji=Z_mat(size_scalar+(j-1)*(2*nodes_row+1)+(1:2*nodes_row+1),size_scalar+(i-1)...
            *(2*nodes_col+1)+(1:2*nodes_col+1));
        norm_vec(j)=norm_vec(j)+InfNuNorm_mat(Zji)*norm_weight(j)/norm_weight(i);
    end
end
